...
import 'package:nostr_core/nostr/nips/nip_010.dart';
  ...
  static ChannelMessage getChannelMessage(Event event) {
    try {
      if (event.kind == 42) {
        ...
        Thread thread = Nip10.fromTags(event.stTags);
  ...
  static ChannelMessageHidden getMessageHidden(Event event) {
    try {
      if (event.kind == 43) {
        ...
        return ChannelMessageHidden(event.pubkey,
            tagsToMessageId(event.stTags)!, reason, event.createdAt);
      }
